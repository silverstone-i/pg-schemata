...
- **`touch()` requires audit fields** and rejects with `SchemaDefinitionError` when they are disabled. It previously produced the unrelated `DTO must be a non-empty object` from two frames down
...
- **`constraints.primaryKey` is documented as DDL-only.** It generates the `PRIMARY KEY` constraint; it does not determine which column the by-id methods match on. `findById`, `update`, `delete`, `bulkUpdate`, and the soft-delete helpers all emit `WHERE id = $1` against a column literally named `id`, so **composite primary keys are not supported by those methods** — the constraint is created correctly but a by-id call still matches on `id` alone. `bulkUpdate` even reads the declared primary key and then targets `id` anyway. Recorded in the schema-definition guide, the schema-types and table-model references, and the PRD; making the CRUD layer key-agnostic is deferred, since it changes the signature of every by-id method
