...
- **`json`/`jsonb` columns marked `notNull` are now actually required on insert** — they mapped to `z.any()`, which makes the object key optional in zod, so a `jsonb NOT NULL` column passed validation with the key absent entirely
...
- **`NOT NULL` columns reject `null` in every mapped type.** `z.coerce.date()` did not merely accept `null`, it coerced it to the Unix epoch, so a `NOT NULL timestamptz` with no default validated `null`; `json`/`jsonb` had the same hole for an explicit `null`
