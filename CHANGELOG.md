...
- **The where-modifiers guide warns that an object carrying `$and` or `$or` contributes only that group.** Any ordinary column key beside it is silently dropped, so `{ $or: [...], tenant_id }` is not scoped to the tenant. `FiltersInput` permits the shape and nothing flagged it. The guide now shows the sibling-object form that emits both
