...
- **[prd/plans/deferred-correctness-followups.md](prd/plans/deferred-correctness-followups.md)** records the three review findings deliberately not taken — the ColumnSet cache key, composite primary keys, and repository type augmentation — with the reasoning, a reproduction, and a suggested approach for each
