...
- **`DB.close()`** closes the default instance, clears `DB.db` / `DB.pgp` and any resolver registered through `DB.init()`, and leaves the singleton ready for a fresh `init()`. Safe before initialization and when called repeatedly or concurrently
