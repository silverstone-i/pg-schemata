...
describe('QueryModel', () => {
  ...
  describe('findAfterCursor', () => {
    ...
    // Characterization tests written before the 2.0.0 removal of the
    // lowercase and/or branch: they pin the SQL produced for $and/$or and
    // must pass unchanged after the removal.
    ...
    test('removed lowercase and keys are treated as column names, not boolean groups', async () => {
      mockDb.any.mockResolvedValue([]);
      await expect(
        model.findAfterCursor({ id: 2 }, 1, ['id'], {
          filters: { and: [{ email: 'a@x.com' }] },
        })
      ).rejects.toThrow();
    });
  });
