...
function createColumnSet(
  ...
  const columns = columnsetColumns
    .map(col => {
      ...
      // Skip auto-generated columns: any serial spelling, or a UUID primary
      // key with a default. Normalized rather than compared literally — the
      // validator has always accepted `SERIAL` and now accepts ` serial `
      // too, and a check that disagreed with it left those columns in the
      // ColumnSet while still being auto-generated by the database.
      const normalizedType = normalizeSqlType(col.type);
      if (
        isSerialType(normalizedType) ||
        (normalizedType === 'uuid' && isPrimaryKey && hasDefault)
      ) {
        return null;
      }
